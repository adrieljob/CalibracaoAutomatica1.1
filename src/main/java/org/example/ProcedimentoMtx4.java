...
@RestController
public class ProcedimentoMtx4 {

	private static final String RESET = "\u001B[0m";
	private static final String MAGENTA = "\u001B[35m";
	private static boolean procedimentoAtivoMtx4 = false;
	private static LocalDateTime horaInicioProcedimentoMtx4 = null;
	private static Map<String, Object> statusProcedimentoMtx4 = new ConcurrentHashMap<>();

	@Autowired
	private RestTemplate restTemplate;

	@GetMapping("/status-procedimento-mtx4")
	public ResponseEntity<Map<String, Object>> getStatusProcedimentoMtx4() {
		Map<String, Object> resposta = new HashMap<>();

		resposta.put("ativo", procedimentoAtivoMtx4);
		resposta.put("hora_inicio", horaInicioProcedimentoMtx4 != null ? horaInicioProcedimentoMtx4.toString() : null);
		resposta.put("progresso", statusProcedimentoMtx4);
		resposta.put("ultima_atualizacao", LocalDateTime.now().toString());

		// Verificar timeout (2 horas para procedimento completo)
		if (procedimentoAtivoMtx4 && horaInicioProcedimentoMtx4 != null) {
			Duration duracao = Duration.between(horaInicioProcedimentoMtx4, LocalDateTime.now());
			if (duracao.toHours() > 2) {
				procedimentoAtivoMtx4 = false;
	...
	@PostMapping("/executar-procedimento-completo-mtx4")
	public ResponseEntity<Map<String, Object>> executarProcedimentoCompletomtx4() {
		Map<String, Object> respostaGeral = new HashMap<>();
		Map<String, Object> resultados = new HashMap<>();

		try {
			System.out.println(MAGENTA + "=== INICIANDO PROCEDIMENTO COMPLETO MTX4 ===" + RESET);
			...
			for (int i = 0; i < canais.length; i++) {
				...
				if (!"sucesso".equals(resultadoAjuste.get("status"))) {
					...
					System.out.println(MAGENTA + "Ajuste offset concluído para canal " + canal + RESET);
				}
				...
				if (!"sucesso".equals(statusLinearizacao) && !"parcial".equals(statusLinearizacao)) {
					...
					System.out.println(MAGENTA + "✓ Linearização concluída para canal " + canal + RESET);
				...
				if ("sucesso".equals(statusChecagem)) {
					System.out.println(MAGENTA + "✓ Checagem final concluída para canal " + canal + RESET);
				...
				if (i < canais.length - 1) {
					System.out.println(MAGENTA + "\n" + "=".repeat(50) + RESET);
					System.out.println(MAGENTA + "AGUARDANDO 30 SEGUNDOS ANTES DO PRÓXIMO CANAL..." + RESET);
			...
			System.out.println(MAGENTA + "\n" + "=".repeat(60) + RESET);
			System.out.println(MAGENTA + "EXECUTANDO CHECAGEM FINAL GERAL APÓS TODOS OS CANAIS" + RESET);
			System.out.println(MAGENTA + "=".repeat(60) + RESET);
	...
	@PostMapping("/executar-procedimento-canal-mtx4")
	public ResponseEntity<Map<String, Object>> executarProcedimentoCanalmtx4(@RequestParam String canal) {
		Map<String, Object> resposta = new HashMap<>();

		// MARCA COMO ATIVO
		procedimentoAtivoMtx4 = true;
		horaInicioProcedimentoMtx4 = LocalDateTime.now();
		statusProcedimentoMtx4.clear();
		statusProcedimentoMtx4.put("canal", canal);
		statusProcedimentoMtx4.put("status", "executando");
		statusProcedimentoMtx4.put("etapa", "ajuste_offset");

		try {
			System.out.println(MAGENTA + "=== INICIANDO PROCEDIMENTO PARA CANAL: " + canal + " ===" + RESET);
			System.out.println(MAGENTA + "Hora de início: " + LocalDateTime.now() + RESET);
			...
			resposta.put("hora_inicio", horaInicioProcedimentoMtx4.toString());
			...
			resposta.put("hora_inicio", horaInicioProcedimentoMtx4 != null ? horaInicioProcedimentoMtx4.toString() : LocalDateTime.now().toString());
			resposta.put("canal", canal);
			...
			procedimentoAtivoMtx4 = false;
			statusProcedimentoMtx4.put("status", "finalizado");
		}
	}
	...
	private Map<String, Object> chamarAjusteOffset(String canal) {
		try {
			...
			statusProcedimentoMtx4.put("canal", canal);
			try {
				System.out.println(MAGENTA + "  Chamando endpoint de ajuste offset para canal " + canal + RESET);

				// URL do endpoint existente do AjustarOffSetMtx4
				String url = "http://localhost:8087/executar-offset-canal-mtx4?canal=" + canal;
				...
				if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
					...
					System.out.println(MAGENTA + "  Resposta do ajuste offset: " + resultado.get("status") + RESET);
					return resultado;
				} else {
	...
	private Map<String, Object> chamarLinearizacao(String canal) {
		try {
			statusProcedimentoMtx4.put("etapa", "linearizacao");
			try {
				System.out.println(MAGENTA + "  Chamando endpoint de linearização para canal " + canal + RESET);

				// URL do endpoint existente do LinearizacaoMtx4
				String url = "http://localhost:8087/executar-linearizacao-canal-mtx4?canal=" + canal;
				...
				if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
					resultado = response.getBody();
					System.out.println(MAGENTA + "  Resposta da linearização: " + resultado.get("status") + RESET);
					return resultado;
				} else {
	...
	private Map<String, Object> chamarChecagem() {
		try {
			statusProcedimentoMtx4.put("etapa", "checagem_final");

			System.out.println(MAGENTA + "  Chamando endpoint de checagem final" + RESET);
			...
			if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
				resultado = response.getBody();
				System.out.println(MAGENTA + "  Resposta da checagem final: " + resultado.get("status") + RESET);
	...
	@PostMapping("/cancelar-procedimento-mtx4")
	public ResponseEntity<Map<String, Object>> cancelarProcedimentomtx4() {
		Map<String, Object> resposta = new HashMap<>();

		try {
			System.out.println(MAGENTA + "Solicitação de cancelamento de procedimento recebida"+ RESET);

			// Opcional: Chamar endpoints de cancelamento dos serviços
			try {
				restTemplate.postForEntity("http://localhost:8087/cancelar-linearizacao-mtx4", null, Map.class);
			} catch (Exception e) {
				System.err.println("Erro ao cancelar linearização: " + e.getMessage());
			}

			try {
				restTemplate.postForEntity("http://localhost:8087/cancelar-offset-mtx4", null, Map.class);
			} catch (Exception e) {
				System.err.println("Erro ao cancelar linearização: " + e.getMessage());
			}
