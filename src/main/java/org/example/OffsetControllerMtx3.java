...
@RestController
@RequestMapping("/api/offset/mtx3")
public class OffsetControllerMtx3 {
    ...
    private Map<String, Object> configurarOffsetMTX(String mtxNumber, String valorOffset, String urlBase) {
        ...
        try {
            ...
            // Tentar diferentes estratégias para encontrar Internal3
            WebElement Internal3 = null;
            String[] xpaths = {
                    "//label[contains(@link, 'Internal3')]/input",
                    "//input[contains(@id, 'Internal3')]",
                    "//*[contains(text(), 'Internal3')]/preceding-sibling::input",
                    "//input[contains(@name, 'Internal3')]"
            };

            for (String xpath : xpaths) {
                try {
                    Internal3 = driver.findElement(By.xpath(xpath));
                    if (Internal3 != null && Internal3.isDisplayed() && Internal3.isEnabled()) {
                        System.out.println(GREEN + "Internal3 encontrado com xpath: " + xpath + RESET);
                        break;
                    }
                } catch (Exception e) {
                    continue;
            ...
            if (Internal3 == null) {
                throw new Exception("Elemento Internal3 não encontrado");
            }

            Internal3.click();
            Thread.sleep(500);
            System.out.println(GREEN + "Internal 3 selecionado" + RESET);
