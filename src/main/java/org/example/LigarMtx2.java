...
public class LigarMtx2 {
    ...
    private Map<String, String> executarAcao(String valor) {
        ...
        Map<String, Object> resultadoDetalhado = configurarStatusMTX2(valor);
        ...
        if (resultadoDetalhado.containsKey("mensagem")) {
            ...
            resultado.put("mensagem", "MTX2 ligado");
            resultado.put("acao", "LIGADO");
        } else {
            resultado.put("mensagem", "MTX2 desligado");
    ...
    private Map<String, Object> configurarStatusMTX2(String valorStatus) {
        String urlBase = "http://10.10.103.103/debug/";
        System.out.println(YELLOW + "URL que será acessada: " + urlBase + RESET);
        ...
        try {
            ...
            try {
                ...
                RfMasterOn = wait.until(ExpectedConditions.elementToBeClickable(
                        By.id("PowerAmplifier2.Config.RfMasterOn")));
                System.out.println(YELLOW + "RfMasterOn encontrado pelo ID exato" + RESET);
            } catch (Exception e1) {
                try {
                    // Tentativa 2: Pelo ID com underscore
                    RfMasterOn = wait.until(ExpectedConditions.elementToBeClickable(
                            By.id("PowerAmplifier2_Config_RfMasterOn")));
                    System.out.println(YELLOW + "RfMasterOn encontrado pelo ID com underscore" + RESET);
                } catch (Exception e2) {
                    try {
                        // Tentativa 3: Qualquer elemento com RfMasterOn
                        RfMasterOn = wait.until(ExpectedConditions.elementToBeClickable(
                                By.xpath("//*[contains(@id, 'RfMasterOn') or contains(@name, 'RfMasterOn')]")));
                        System.out.println(YELLOW + "RfMasterOn encontrado por busca genérica" + RESET);
                    } catch (Exception e3) {
            ...
            RfMasterOn.click();
            Thread.sleep(300);
            System.out.println(YELLOW + "RfMasterOn clicado - diálogo aberto" + RESET);
            ...
            try {
                ...
                if (campoNewValue.getTagName().equals("input") || campoNewValue.getTagName().equals("textarea")) {
                    System.out.println(YELLOW + "Campo ativo encontrado");
                } else {
                    campoNewValue = null;
                }
            } catch (Exception e) {
                System.out.println(YELLOW + "Campo ativo não encontrado" + RESET);
            }
                ...
                try {
                    ...
                    for (WebElement input : todosInputs) {
                        if (input.isDisplayed() && input.isEnabled()) {
                            campoNewValue = input;
                            System.out.println(YELLOW + "Campo input encontrado" + RESET);
                            break;
                        }
                    }
                } catch (Exception e) {
                    System.out.println(YELLOW + "Nenhum input encontrado" + RESET);
                }
            }
            ...
            if (campoNewValue != null) {
                ...
                System.out.println(YELLOW + "Valor " + valorStatus + " digitado" + RESET);
                Thread.sleep(300);
            ...
            if (driver != null) {
                driver.quit();
                System.out.println(YELLOW + "Driver finalizado" + RESET + RESET);
            }
        }
