...
public class AjustarOffSetMtx1 {
    ...
    // Códigos de cores para console (GREEN para MTX1)
    ...
    public ResponseEntity<Map<String, Object>> executarRotinaCompleta() {
        ...
        try {
            System.out.println(GREEN + "=== INICIANDO ROTINA COMPLETA MTX1 ===" + RESET);
            ...
            for (int i = 0; i < canais.length; i++) {
                ...
                System.out.println(GREEN + "PROCESSANDO CANAL MTX1: " + canal + RESET);
                ...
                if (!"sucesso".equals(resultadoCanal.get("status"))) {
                    throw new RuntimeException("Falha no canal MTX1 " + canal + ": " + resultadoCanal.get("mensagem"));
                }
            ...
            respostaGeral.put("mensagem", "Rotina completa MTX1 executada com sucesso");
            ...
            System.out.println(GREEN + "\n=== ROTINA COMPLETA MTX1 FINALIZADA ===" + RESET);
            ...
            respostaGeral.put("mensagem", "Erro na rotina completa MTX1: " + e.getMessage());
            respostaGeral.put("hora_inicio", LocalDateTime.now().toString());
            respostaGeral.put("resultados", resultados);

            System.err.println("Erro na rotina completa MTX1: " + e.getMessage());
            e.printStackTrace();
    ...
    private Map<String, Object> processarCanalCompleto(WebDriver driver, WebDriverWait wait, String canal) {
        ...
        try {
            // ========== ETAPA 1: MUDAR CANAL ==========
            System.out.println(GREEN + "\n[ETAPA 1] Mudando para canal MTX1: " + canal + RESET);
            String canalAntes = mudarCanal(driver, wait, canal);
            Thread.sleep(2000);

            // ========== ETAPA 2: AJUSTAR OFFSET (parte inicial) ==========
            System.out.println(GREEN + "\n[ETAPA 2] Configurando offset e potência do MTX1" + RESET);
            ...
            System.out.println(GREEN + "  2.5. Aguardando 1 minuto para estabilização..." + RESET);
            Thread.sleep(60000);

            // ========== ETAPA 3: VERIFICAR E AJUSTAR DINAMICAMENTE ==========
            System.out.println(GREEN + "\n[ETAPA 3] Verificando e ajustando dinamicamente o MTX1" + RESET);

            // Checa o canal (apenas para confirmar)
            String canalAtual = verificarCanal(driver, wait);
            System.out.println(GREEN + "  Canal atual MTX1: " + canalAtual + RESET);
            ...
            if (!"sucesso".equals(resultadoAjuste.get("status"))) {
                throw new Exception("Falha no ajuste dinâmico MTX1: " + resultadoAjuste.get("mensagem"));
            }

            // ========== ETAPA 4: COLETAR RESULTADOS FINAIS ==========
            System.out.println(GREEN + "\n[ETAPA 4] Coletando resultados finais do MTX1" + RESET);
            ...
            resultado.put("mensagem", "Canal MTX1 " + canal + " processado com sucesso");
            ...
            System.err.println("Erro no processamento do canal MTX1 " + canal + ": " + e.getMessage());
            e.printStackTrace();

            resultado.put("status", "erro");
            resultado.put("mensagem", "Erro no canal MTX1 " + canal + ": " + e.getMessage());
    ...
    private void chamarFuncaoCancelamento(String canal, Map<String, Object> resultadoCanal) {
        try {
            System.out.println(GREEN + "\n=== CHAMANDO CANCELAMENTO PARA CANAL MTX1 " + canal + " ===" + RESET);
            ...
            respostaCancelamento.put("mensagem", "Cancelamento chamado para canal MTX1 " + canal);
            ...
            System.err.println("Erro ao chamar cancelamento para canal MTX1 " + canal + ": " + e.getMessage());
        }
    }
    ...
    private void salvarLogCancelamento(String canal, Map<String, Object> respostaCancelamento) {
        String filePath = System.getProperty("user.dir") + "/logs_cancelamento_mtx1.txt";
        try (java.io.FileWriter writer = new java.io.FileWriter(filePath, true)) {
            ...
            System.out.println(GREEN + "  Log de cancelamento MTX1 salvo em: " + filePath + RESET);
        } catch (Exception e) {
            System.err.println("Erro ao salvar log de cancelamento MTX1: " + e.getMessage());
        }
    }
    ...
    private Map<String, Object> executarAjusteDinamicoPorCanal(WebDriver driver, WebDriverWait wait, String canal) throws Exception {
        ...
        // LER OFFSET ATUAL DO EQUIPAMENTO MTX1
        int offsetAtual = lerOffsetAtual(driver, wait);
        System.out.println(GREEN + "  Offset inicial lido do equipamento MTX1: " + offsetAtual + RESET);
        ...
        // Definir limites de corrente para cada canal do MTX1
        switch (canal) {
            case "14":
                correnteMinima = 69;
                correnteMaximaErro = 71;
                System.out.println(GREEN + "  Parâmetros para canal 14 do MTX1: 69-71 A" + RESET);
                break;
            case "34":
                correnteMinima = 64;
                correnteMaximaErro = 66;
                System.out.println(GREEN + "  Parâmetros para canal 34 do MTX1: 64-66 A" + RESET);
                break;
            case "51":
                correnteMinima = 59;
                correnteMaximaErro = 61;
                System.out.println(GREEN + "  Parâmetros para canal 51 do MTX1: 59-61 A" + RESET);
                break;
            default:
                throw new Exception("Canal não suportado no MTX1: " + canal);
        }
        ...
        while (iteracoes < maxIteracoes) {
            ...
            System.out.println(GREEN + "    Corrente atual MTX1: " + corrente + " A" + RESET);

            // Verificação especial se corrente for zero
            if (corrente == 0) {
                System.out.println(GREEN + "    Corrente = 0 A. Verificando potência e corrente completa do MTX1..." + RESET);
            ...
            if (corrente >= correnteMinima) {
                ...
                System.out.println(GREEN + "    Corrente após 10s: " + corrente + " A" + RESET);
                ...
                if (corrente > correnteMaximaErro) {
                    ...
                    if (ajustesFeitos > maxAjustes) {
                        String erroFinal = "ERRO MTX1: Máximo de ajustes (" + maxAjustes + ") atingido. Corrente ainda alta: " + corrente + " A";
                    ...
                    if (offsetAtual + 5 > offsetMaximo) {
                        String erroFinal = "ERRO MTX1: Offset máximo (" + offsetMaximo + ") atingido. Corrente ainda alta: " + corrente + " A";
                        System.err.println("    " + erroFinal);
                        resultado.put("status", "erro");
                        resultado.put("mensagem", erroFinal);
                        resultado.put("iteracoes", iteracoes);
                    ...
                    String erroMsg = "Corrente MTX1 " + corrente + " A > limite máximo " + correnteMaximaErro + " A. Ajustando offset...";
                    ...
                    System.out.println(GREEN + "    Aplicando novo offset " + offsetAtual + " no MTX1..." + RESET);
                ...
                System.out.println(GREEN + "    SUCESSO MTX1: Corrente " + corrente + " A dentro dos limites (" +
                        correnteMinima + "-" + correnteMaximaErro + " A)" + RESET);
                resultado.put("status", "sucesso");
                resultado.put("mensagem", "Corrente MTX1 ajustada corretamente");
            ...
            offsetAtual--;
            System.out.println(GREEN + "    Reduzindo offset MTX1 para: " + offsetAtual + RESET);

            // Verificar limite mínimo de offset
            if (offsetAtual < -50) {
                String erroMsg = "ERRO MTX1: Offset chegou a -50 e corrente ainda não atingiu " + correnteMinima + " A";
        ...
        String erroMsg = "ERRO MTX1: Máximo de " + maxIteracoes + " iterações atingido";
        System.err.println("  " + erroMsg);
        resultado.put("status", "erro");
        resultado.put("mensagem", erroMsg);
        resultado.put("iteracoes", iteracoes);
    ...
    private int lerOffsetAtual(WebDriver driver, WebDriverWait wait) throws Exception {
        try {
            System.out.println(GREEN + "  Lendo offset atual do equipamento MTX1..." + RESET);
            ...
            System.out.println(GREEN + "    Texto do offset atual MTX1: " + textoOffset + RESET);

            double offsetValor = extrairValorNumerico(textoOffset);
            System.out.println(GREEN + "    Offset atual MTX1 extraído: " + (int)offsetValor + RESET);

            return (int) offsetValor;

        } catch (Exception e) {
            System.err.println("Erro ao ler offset atual do MTX1: " + e.getMessage());
            return 0; // Retorna 0 como fallback
        }
    }

    // Altera o canal do equipamento MTX1
    private String mudarCanal(WebDriver driver, WebDriverWait wait, String canal) throws Exception {
        try {
            ...
            System.out.println(GREEN + "  Canal atual MTX1: " + canalAntes + RESET);

            // Se já estiver no canal correto, retornar
            if (canalAntes.equals(canal)) {
                System.out.println(GREEN + "  MTX1 já está no canal " + canal + RESET);
                return canalAntes;
            }
            ...
            if (!sucesso) {
                throw new Exception("Não foi possível mudar o canal do MTX1");
            }
            ...
            System.out.println(GREEN + "  Canal MTX1 configurado: " + canalDepois + RESET);

            int tentativas = 0;
            while (!canalDepois.equals(canal) && tentativas < 3) {
                System.out.println(GREEN + "  Canal MTX1 não mudou corretamente. Tentativa " + (tentativas + 1) + RESET);
                Thread.sleep(1000);
            ...
            if (!canalDepois.equals(canal)) {
                System.err.println("  AVISO: Canal MTX1 não mudou corretamente. Esperado: " + canal + ", Lido: " + canalDepois);
            }

            return canalAntes;

        } catch (Exception e) {
            System.err.println("  Erro ao mudar canal do MTX1: " + e.getMessage());
            throw e;
        }
    }
    ...
    private void desligarMTX1(WebDriver driver, WebDriverWait wait) throws Exception {
        ...
        if (!configurarValor(driver, rfMasterOn, "2")) {
            throw new Exception("Falha ao desligar RfMasterOn do MTX1");
        }
    ...
    private void ligarMTX1(WebDriver driver, WebDriverWait wait) throws Exception {
        ...
        if (!configurarValor(driver, rfMasterOn, "1")) {
            throw new Exception("Falha ao ligar RfMasterOn do MTX1");
        }
    ...
    // Configura threshold de proteção do MTX1
    private void configurarThershold(WebDriver driver, WebDriverWait wait, String valorThershold) throws Exception {
        ...
        if (!configurarValor(driver, offset, valorThershold)) {
            throw new Exception("Falha ao configurar thershold do MTX1 para " + valorThershold);
        }

        System.out.println(GREEN + "  Thershold MTX1 configurado: " + valorThershold + RESET);
        Thread.sleep(300);
    }

    // Configura offset de potência do MTX1
    private void configurarOffset(WebDriver driver, WebDriverWait wait, String valorOffset) throws Exception {
        ...
        if (!configurarValor(driver, offset, valorOffset)) {
            throw new Exception("Falha ao configurar offset do MTX1 para " + valorOffset);
        }

        System.out.println(GREEN + "  Offset MTX1 configurado: " + valorOffset + RESET);
        Thread.sleep(300);
    }

    // Configura potência de saída do MTX1
    private void configurarPotencia(WebDriver driver, WebDriverWait wait, String potencia) throws Exception {
        ...
        if (!configurarValor(driver, outputPower, potencia)) {
            throw new Exception("Falha ao configurar potência do MTX1");
        }

        System.out.println(GREEN + "  Potência MTX1 configurada: " + potencia + RESET);
        Thread.sleep(300);
    }

    // Método genérico para configurar valores em campos de entrada do MTX1
    private boolean configurarValor(WebDriver driver, WebElement elemento, String novoValor) {
        try {
            ...
            System.err.println("Erro ao configurar valor no MTX1: " + e.getMessage());
            e.printStackTrace();
        }
        return false;
    }

    // Lê potência atual do equipamento MTX1
    ...
    // Lê corrente atual do equipamento MTX1
    private String verificarCorrente(WebDriver driver, WebDriverWait wait) throws Exception {
        ...
        System.out.println(GREEN + "    Texto completo da corrente MTX1: " + textoCompleto + RESET);

        double correnteValor = extrairValorNumerico(textoCompleto);

        System.out.println(GREEN + "    Corrente MTX1 extraída: " + correnteValor + " A" + RESET);

        return String.valueOf(correnteValor);
    }

    // Lê canal atual do equipamento MTX1
    private String verificarCanal(WebDriver driver, WebDriverWait wait) throws Exception {
        try {
            ...
            System.out.println(GREEN + "    Texto do elemento canal MTX1: " + canalTexto + RESET);
            ...
            if (canalTexto == null || canalTexto.isEmpty() || canalTexto.contains("Modulator1.Config.UpConverter.ChannelNumber")) {
                ...
                System.out.println(GREEN + "    Value attribute do canal MTX1: " + canalTexto + RESET);
            }

            if (canalTexto == null || canalTexto.isEmpty()) {
                canalTexto = canalElement.getAttribute("innerText");
                System.out.println(GREEN + "    InnerText do canal MTX1: " + canalTexto + RESET);
            }
            ...
            if (canalTexto != null && !canalTexto.isEmpty()) {
                ...
                if (!numeros.isEmpty()) {
                    System.out.println(GREEN + "    Canal MTX1 extraído: " + numeros + RESET);
                    return numeros;
                }
            }

            System.out.println(GREEN + "    Não conseguiu extrair canal MTX1, retornando N/A" + RESET);
            return "N/A";

        } catch (Exception e) {
            System.err.println("    Erro ao verificar canal MTX1: " + e.getMessage());
            return "N/A";
        }
    }
    ...
    private String verificarPotenciaEcorrente(WebDriver driver, WebDriverWait wait) throws Exception {
        ...
        System.out.println(GREEN + "    Potência MTX1 extraída: " + potenciaValor + " W" + RESET);
        ...
        System.out.println(GREEN + "    Texto completo da corrente MTX1: " + textoCorrente + RESET);
        ...
        if (correnteValor == 0 && potenciaValor == 0) {
            throw new Exception("EQUIPAMENTO MTX1 DESLIGADO - Corrente: " + correnteValor +
                    " A, Potência: " + potenciaValor + " W");
        }

        if (correnteValor == 0) {
            System.out.println(GREEN + "    Corrente MTX1 zero, retornando valor da potência" + RESET);
            return "Potência MTX1: " + potenciaValor + " W";
        }

        return "Potência MTX1: " + potenciaValor + " W, Corrente MTX1: " + correnteValor + " A";
    }
    ...
    private void salvarLogCanal(String canal, String canalAntes, String canalDepois, String potencia, String corrente, String offset, Map<String, Object> resultadoAjuste) {
        String filePath = System.getProperty("user.dir") + "/logs_rotina_completa_mtx1.txt";
        try (java.io.FileWriter writer = new java.io.FileWriter(filePath, true)) {
            writer.write(LocalDateTime.now() +
                    " | MTX1| Canal: " + canal +
                    " | Antes: " + canalAntes +
            ...
            System.out.println(GREEN + "  Log MTX1 salvo em: " + filePath + RESET);
        } catch (Exception e) {
            System.err.println("Erro ao salvar log MTX1: " + e.getMessage());
        }
    }
    ...
    private void debugElemento(WebElement elemento, String nome) {
        try {
            ...
            System.err.println("Erro no debug MTX1: " + e.getMessage());
        }
    }

    // Endpoint para configurar offset manualmente no MTX1
        ...
        try {
            ...
            if (offsetElement == null) {
                try {
                    ...
                    throw new Exception("Não foi possível encontrar o campo de offset do MTX1");
                }
            }
            ...
            if (!sucesso) {
                throw new Exception("Não foi possível configurar o offset do MTX1");
            }
            ...
            try {
                ...
                System.err.println("Não foi possível ler offset atual do MTX1: " + e.getMessage());
            }

            // Preparar resposta
            resposta.put("status", "sucesso");
            resposta.put("mensagem", "Offset MTX1 configurado com sucesso");
            ...
            System.out.println(GREEN + "=== OFFSET MTX1 CONFIGURADO COM SUCESSO ===" + RESET);

            return ResponseEntity.ok(resposta);

        } catch (Exception e) {
            System.err.println("Erro ao configurar offset do MTX1: " + e.getMessage());
            e.printStackTrace();

            resposta.put("status", "erro");
            resposta.put("mensagem", "Erro no MTX1: " + e.getMessage());
    ...
    // Endpoint para executar ajuste em um canal específico do MTX1
    ...
    // Endpoint para cancelar operação (apenas simbólico) do MTX1
    @PostMapping("/cancelar-offset-mtx1")
    public ResponseEntity<Map<String, Object>> cancelarOffsetMtx1() {
        Map<String, Object> resposta = new HashMap<>();

        System.out.println(GREEN + "Solicitação de cancelamento de offset MTX1 recebida" + RESET);

        resposta.put("status", "sucesso");
        resposta.put("mensagem", "Solicitação de cancelamento MTX1 recebida");
