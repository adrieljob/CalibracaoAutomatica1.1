...
@RestController
@RequestMapping("/api/offset/mtx4")
public class OffsetControllerMtx4 {
    ...
    private Map<String, Object> configurarOffsetMTX(String mtxNumber, String valorOffset, String urlBase) {
        ...
        try {
            ...
            // Tentar diferentes estratégias para encontrar Internal4
            WebElement Internal4 = null;
            String[] xpaths = {
                    "//label[contains(@link, 'Internal4')]/input",
                    "//input[contains(@id, 'Internal4')]",
                    "//*[contains(text(), 'Internal4')]/preceding-sibling::input",
                    "//input[contains(@name, 'Internal4')]"
            };

            for (String xpath : xpaths) {
                try {
                    Internal4 = driver.findElement(By.xpath(xpath));
                    if (Internal4 != null && Internal4.isDisplayed() && Internal4.isEnabled()) {
                        System.out.println(GREEN + "Internal4 encontrado com xpath: " + xpath + RESET);
                        break;
                    }
                } catch (Exception e) {
                    continue;
            ...
            if (Internal4 == null) {
                throw new Exception("Elemento Internal4 não encontrado");
            }

            Internal4.click();
            Thread.sleep(500);
            System.out.println(GREEN + "Internal 4 selecionado" + RESET);
