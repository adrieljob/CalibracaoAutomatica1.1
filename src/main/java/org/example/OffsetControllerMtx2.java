...
@RestController
@RequestMapping("/api/offset/mtx2")
public class OffsetControllerMtx2 {
    ...
    private Map<String, Object> configurarOffsetMTX(String mtxNumber, String valorOffset, String urlBase) {
        ...
        try {
            ...
            // Tentar diferentes estratégias para encontrar Internal2
            WebElement Internal2 = null;
            String[] xpaths = {
                    "//label[contains(@link, 'Internal2')]/input",
                    "//input[contains(@id, 'Internal2')]",
                    "//*[contains(text(), 'Internal2')]/preceding-sibling::input",
                    "//input[contains(@name, 'Internal2')]"
            };

            for (String xpath : xpaths) {
                try {
                    Internal2 = driver.findElement(By.xpath(xpath));
                    if (Internal2 != null && Internal2.isDisplayed() && Internal2.isEnabled()) {
                        System.out.println(GREEN + "Internal2 encontrado com xpath: " + xpath + RESET);
                        break;
                    }
                } catch (Exception e) {
                    continue;
            ...
            if (Internal2 == null) {
                throw new Exception("Elemento Internal2 não encontrado");
            }

            Internal2.click();
            Thread.sleep(500);
            System.out.println(GREEN + "Internal 2 selecionado" + RESET);
